...
const createDMMSettings = async (req, res) => {
    try {
        ...
        if (record) {
            ...
            if (section === 'all') {
                // Handle Save All - update operation data and append shift parameter if provided
                // Update operation data (shifts) if provided
                if (req.body.shifts) {
                    if (!record.shifts) {
                        record.shifts = {
                    ...
                    ['shift1', 'shift2', 'shift3'].forEach(shiftKey => {
                        if (req.body.shifts[shiftKey]) {
                            ...
                            if (shiftPayload.operatorName !== undefined && String(shiftPayload.operatorName).trim() !== '') {
                                record.shifts[shiftKey].operatorName = String(shiftPayload.operatorName).trim();
                            }
                            if (shiftPayload.checkedBy !== undefined && String(shiftPayload.checkedBy).trim() !== '') {
                ...
                // Append shift parameter if provided and shift is selected
                if (req.body.selectedShift && req.body.shiftParameters) {
                    const shiftKey = `shift${req.body.selectedShift}`;
                    const shiftParams = req.body.shiftParameters;
                    ...
                    if (!record.parameters[shiftKey]) {
                        record.parameters[shiftKey] = [];
                    }
                    
                    // Calculate next sNo
                    const existingEntries = record.parameters[shiftKey] || [];
                    ...
                    record.parameters[shiftKey].push(newEntry);
                }
